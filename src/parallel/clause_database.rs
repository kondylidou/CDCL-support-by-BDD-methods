...
use crate::{
    add_assumptions_to_glucose_solver, add_incoming_clause_to_clauses_vec, GlucoseWrapper,
};
use bit_set::BitSet;
...
impl ClauseDatabase {
    pub fn new() -> ClauseDatabase {
        ...
        let filter_local = ClassicBloomFilter::new(
            100,
            0.03,
            DefaultBuildHashKernels::new(random(), RandomState::new()),
        );
        let filter_global = ClassicBloomFilter::new(
            100,
            0.03,
            DefaultBuildHashKernels::new(random(), RandomState::new()),
        );
    ...
    fn insert_to_bloom_filter_local(&mut self, clause: &Vec<i32>) {
        clause
            .iter()
            .for_each(|i| self.local_filter_bloom.insert(i));
    }

    fn insert_to_bloom_filter_global(&mut self, clause: &Vec<i32>) {
        clause
            .iter()
            .for_each(|i| self.global_filter_bloom.insert(i));
    }
    ...
    pub fn send(
        &mut self,
        clause_input: Vec<i32>,
        solver_wrapper: GlucoseWrapper,
        stats: &mut Stats,
    ) {
        stats.add_sent_bdd();
...
const PRIMES: [i64; 12] = [
    2038072819, 2038073287, 2038073761, 2038074317, 2038072823, 2038073321, 2038073767, 2038074319,
