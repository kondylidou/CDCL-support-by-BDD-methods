...
std::string getPathToSimp()
{
    // Assuming exec is in the simp directory.
    return "./";
}
...
void vectorToPython(Solver::ListForInstances list)
{
    ...
    std::string pathToPlotter = getPathToSimp();
    std::string sysPathAppendCommand = "sys.path.append(\"" + pathToPlotter + "\")";
    PyRun_SimpleString(sysPathAppendCommand.c_str());
